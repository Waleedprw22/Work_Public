...
def camera_to_image(intrinsics, camera_points):
    ...
    for i in prange(camera_points.shape[0]):
        image_coordinates[i, 0] = np.round(int(np.round((camera_points[i, 0] * fu / camera_points[i, 2]) + u0))) # (x * fu/ z) + u0 = u
        image_coordinates[i, 1] = np.round(int(np.round((camera_points[i, 1] * fv / camera_points[i, 2]) + v0))) # (y * fv/z) + v0 =v
...
def depth_to_point_cloud(intrinsics, depth_image):
    ...
    """
    # h corresponds to u, w corresponds to v
     # initializing 3D points
    #We know Z = w. #Find camera origin
    u0 = intrinsics[0, 2]
    v0 = intrinsics[1, 2]
    fu = intrinsics[0, 0]
    fv = intrinsics[1, 1]
    ...
    for i in range(0,h):
        for j in range(0, w):
            if depth_image[i,j] >0:
                ...
                X = (j- u0) *( depth_image[i,j]/fu)
                Y = (i- v0) *( depth_image[i,j]/fv)
                Z = depth_image[i,j]
                points.append([X, Y, Z])
